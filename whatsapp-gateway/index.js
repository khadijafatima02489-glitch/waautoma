...
async function forwardIncoming(rid, msg) {
  ...
  catch (error) { logger.error(`forward failed: ${error.message}`); }
}
...
async function startSession(rid) {
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === "close") {
      ...
      if (code === DisconnectReason.loggedOut) { session.status = "disconnected"; session.qr = null; session.number = null; try { fs.rmSync(sessionDir(rid), { recursive: true, force: true }); } catch (_) {} }
      else { session.status = "connecting"; setTimeout(() => startSession(rid).catch(() => {}), 2500); }
    }
  });
  sock.ev.on("messages.upsert", async (event) => {
    ...
    for (const message of event.messages) {
      ...
      await forwardIncoming(rid, { phone: phoneFromJid(jid), text: text.trim(), messageId: message.key.id, pushName: message.pushName || null, timestamp: new Date().toISOString() });
    }
  });
  return session;
}
...
function resumeSaved() { let dirs = []; try { dirs = fs.readdirSync(SESSIONS_DIR); } catch (_) {} for (const dir of dirs) if (fs.existsSync(path.join(SESSIONS_DIR, dir, "creds.json"))) startSession(dir).catch(() => {}); }
