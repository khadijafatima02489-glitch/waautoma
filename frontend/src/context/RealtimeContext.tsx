...
export function RealtimeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    events.onmessage = (event) => { try { const payload = JSON.parse(event.data) as { type: string }; if (["new_order", "order_update"].includes(payload.type)) { void queryClient.invalidateQueries({ queryKey: ["orders"] }); void queryClient.invalidateQueries({ queryKey: ["analytics"] }); } if (payload.type === "message") void queryClient.invalidateQueries({ queryKey: ["simulation"] }); } catch { /* ignore heartbeat */ } };
    return () => events.close();
  }, [session, queryClient]);
