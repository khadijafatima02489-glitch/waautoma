...
import { LayoutDashboard, ShoppingBag, Users, UtensilsCrossed, MessageCircle, Settings, Sparkles, LogOut, Pizza } from "lucide-react";
import { apiGet, type WhatsAppConfig } from "@/lib/api";
import { useAuth } from "@/context/AuthContext";
import { ConnectionBadge } from "@/components/StatusBadge";

const navigation = [["/dashboard", "Dashboard", LayoutDashboard], ["/orders", "Orders", ShoppingBag], ["/customers", "Customers", Users], ["/menu", "Menu", UtensilsCrossed], ["/whatsapp", "WhatsApp", MessageCircle], ["/settings", "Restaurant Settings", Settings], ["/ai-settings", "AI Settings", Sparkles]] as const;
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  return <div data-testid="dashboard-shell" className="flex min-h-screen bg-[#F9FAF8] text-[#1A1D1A]">
    <aside className="hidden w-64 flex-col border-r border-[#E5E7E2] bg-white md:flex">
      ...
      <div className="border-t border-[#E5E7E2] p-4"><button data-testid="logout-button" onClick={logout} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-sm text-[#6E736D] hover:bg-rose-50 hover:text-rose-700"><LogOut size={16} /> Log out</button></div>
    </aside>
